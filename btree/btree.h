...
private:
  // The details of your implementation go here
   ...
   class Node {
      ...
      int size();
      bool isLeaf();
      int findChildPos(const T& element);
      bool hasChildAt(int pos);
      int insertElement(const T& element);
      Node* getChild(int childPos);
      void setParent(Node *parent);
      void setParentPos(int parent);
      int addChild(const T& element);
      T& element(size_t valPos);
      Node* getParent();
      int getParentPos();
      bool hasParent();
      int findElement(const T& element);
      map<int,Node*> getChildren() { return children_; }
      ...
      private:
      vector<T> elements_; //node elements/values

      //map containing nodes children, using a map since it is easier to keep track
      //of what inbetween elements have children. Easier to use a map when not every node 
      //inbetween element has children
      map<int,Node*> children_;  
   ...
   Node* findInsertNode(const T& elem);
};

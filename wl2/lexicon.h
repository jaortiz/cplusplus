...
class Lexicon {
 ...
 private:
    ...
    // FYI - Ubuntu defines LITTLE_ENDIAN and BYTE_ORDER in /usr/include/endian.h    
#ifdef _MSC_VER 
...
{
    if (!edge) return;
    //cout << "in recMapAll" << endl;
    Edge *curEdge;
    if (first) {
	curEdge = edge;
    } else {
//	cout << "about to extend soFar" << endl;
//	cout << "edge->letter " << edge->letter << endl;  // ******* FAILS HERE!! **********
//	cout << "ordToChar(edge->letter) " << ordToChar(edge->letter) << endl;
	soFar += ordToChar(edge->letter); // ******* FAILS HERE!! **********
//	cout << "about to test edge->accept" << endl;
	if (edge->accept) fn(soFar, clientData);
//	cout << "about to test edge->children" << endl;
	if (!edge->children) return;
//	cout << "about to set currEdge" << endl;

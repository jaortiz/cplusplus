...
int SMatrix::operator()(size_type i, size_type j) const throw(MatrixError) {
   ...
   if(ridx_.find(i) != ridx_.cend()) {    //check if the row is in the map
      auto nElements = ridx_.at(i).first + ridx_.at(i).second;
      for(SMatrix::size_type k = ridx_.at(i).first; k < nElements; ++k) {  //loop through columns
         if(cidx_[k] == j) {  //if columns are equal, location found
            val = vals_[k];  
            break;
         }
      }
...
/**
 *Overloading the *= operator on the precondition the matrices are the same size.
 *Takes advantage of sparse matrix representation by transposing matrix b and only multiplying the non-zero elements of a 
...
SMatrix& SMatrix::operator*=(const SMatrix& b) throw(MatrixError) {
   ...
   SMatrix temp = transpose(b);

   for(auto aIt = ridx_.cbegin();aIt != ridx_.cend();++aIt) {    //Loop through a's rows
      auto aElements = aIt->second.first + aIt->second.second;
      int aRow = aIt->first;
      for(auto bIt = temp.ridx_.cbegin();bIt != temp.ridx_.cend();++bIt) { //loop b's "columns"
         int bCol = bIt->first;
         auto bElements = bIt->second.first + bIt->second.second;           
         int val = 0;  
         for(auto i = aIt->second.first; i < aElements; ++i) {
            bool multiplied = false;
            for(auto j = bIt->second.first; j < bElements && !multiplied; ++j) {
               if(cidx_[i] == temp.cidx_[j]) {
                  val += vals_[i] * temp.vals_[j];   
                  multiplied = true;
               } 
            }
         }
         if(val != 0) {
            c.setVal(aRow,bCol,val);
         }
      }
   }
...
bool SMatrix::setVal(size_type row, size_type col, int val) throw(MatrixError) {
   ...
   if(ridx_.find(row) != ridx_.cend()) {   //row already in ridx
      ...
      if(col == cidx_[pos]) {
         if(val == 0) {
            if(ridx_[row].second == 1) {     //check if overriding last element in a row
               bool start = false;
               ...
               ridx_.erase(row);    //removing empty row
               rowOrder_.erase(remove(rowOrder_.begin(),rowOrder_.end(),row),rowOrder_.end());  //removing row number from rowOrder 
                                                                                                   //vector
            } else {
               --ridx_[row].second;    //update number of elements in row
            }
      ...
      if(val != 0) {      //avoid creating new entry if it is a non zero element
         if(k_ > arrSize_) {
            ...
            rowOrder_.push_back(row);  //add row number to rowOrder
         }
      }
...
bool operator==(const SMatrix& a, const SMatrix& b) {
   ...
   } else if(a.ridx_.size() != b.ridx_.size()) {   //if the number of rows are not the same
      equal = false;
   } else if(a.k_ != b.k_) {  //if the number of non zero elements are not equal
      equal = false;
   } else {
      auto bIt = b.ridx_.cbegin();
      for(auto aIt = a.ridx_.cbegin();aIt != a.ridx_.cend() && equal;++aIt) {    //Loop through a's rows
         if(aIt->first != bIt->first) {      //if row numbers arent equal
            equal = false;
         } else if(aIt->second.second != bIt->second.second) { //if number of row elements are equal
            equal = false;
         } else {  
            auto aElements = aIt->second.first + aIt->second.second;
            for(auto i = aIt->second.first; i < aElements && equal; ++i) {    //Loop a's columns
               if(a.cidx_[i] != b.cidx_[i]) {      //if columns arent the same
                  equal = false;
               } else if(a.vals_[i] != b.vals_[i]) {  //if values are the same
                  equal = false;
               }
            }
         } 
...
bool operator!=(const SMatrix& a, const SMatrix& b) {
   ...
   if(a.rows() != b.rows() || a.cols() != b.cols()) {    //If the matrices do not have the 
      ...
      auto bIt = b.ridx_.cbegin();
      for(auto aIt = a.ridx_.cbegin();aIt != a.ridx_.cend() && !notEqual;++aIt) {    //Loop through a's rows
         if(aIt->first != bIt->first) {      //if row numbers arent equal
            notEqual = true;
         } else if(aIt->second.second != bIt->second.second) { //if number of row elements are equal
            notEqual = true;
            ...
            auto aElements = aIt->second.first + aIt->second.second;
            for(auto i = aIt->second.first; i < aElements && !notEqual; ++i) {    //Loop a's columns
               if(a.cidx_[i] != b.cidx_[i]) {      //if columns arent the same
                  notEqual = true;
               } else if(a.vals_[i] != b.vals_[i]) {  //if values are the same
                  notEqual = true;
 ...
 *rows, otherwise there is a size error. Takes advantage of sparse matrix representation by
 *transposing b and only multiplying the non-zero elements.
...
SMatrix operator*(const SMatrix& a, const SMatrix& b) throw(MatrixError) {
   ...
   for(auto aIt = a.ridx_.cbegin();aIt != a.ridx_.cend();++aIt) {    //Loop through a's rows
      auto aElements = aIt->second.first + aIt->second.second;
      int aRow = aIt->first;
      for(auto bIt = temp.ridx_.cbegin();bIt != temp.ridx_.cend();++bIt) { //loop b's "columns"
         int bCol = bIt->first;
         auto bElements = bIt->second.first + bIt->second.second;           
         int val = 0;  
         for(auto i = aIt->second.first; i < aElements; ++i) {    //Loop a's columns
            bool multiplied = false;
            for(auto j = bIt->second.first; j < bElements && !multiplied; ++j) { //loop b's "rows"
               if(a.cidx_[i] == temp.cidx_[j]) {
                  val += a.vals_[i] * temp.vals_[j];   
                  multiplied = true;
               } 
            }
         }
 ...
 *Takes advantage of sparse matrix representation by only adding the non-zero elements
 *of b to a
 *Returns a new matrix equal to a + b
 *
 */
SMatrix operator+(const SMatrix& a, const SMatrix& b) throw(MatrixError) {
   ...
   SMatrix c = a;       //dimensions of new matrix
   
   for(auto it = b.ridx_.cbegin();it != b.ridx_.cend();++it) {   //Loop through b's rows   
         int row = it->first;                                        
         int nElements = it->second.first + it->second.second;
         for(int pos = it->second.first; pos < nElements; ++pos) {   //Loop through b's columns
            int val = c(row,b.cidx_[pos]) + b.vals_[pos];      //adding b to a.
            c.setVal(row,b.cidx_[pos],val);               
         }
   }
...
/**
 *Overloading the '-' operator. Only works when both matrices have equal dimensions
 *Takes advantage of sparse matrix representation by only subtracting the non-zero elements
 *of b from a
 *Returns a new matrix equal to a - b
 *
 */
SMatrix operator-(const SMatrix& a, const SMatrix& b) throw(MatrixError) {
  ...
  for(auto it = b.ridx_.cbegin();it != b.ridx_.cend();++it) {      //Loop through b's rows  
         ...
         int nElements = it->second.first + it->second.second;
         for(int pos = it->second.first; pos < nElements; ++pos) {      //Loop through b's columns
            int val = c(row,b.cidx_[pos]) - b.vals_[pos];       //subtracting b from a.
            c.setVal(row,b.cidx_[pos],val);
         }
      }
   return c;  
...
int SMatrix::value() const {
   ...
   if(ridx_.find(rowPointer_) != ridx_.cend()) {
      auto nElements = ridx_.at(rowPointer_).first + ridx_.at(rowPointer_).second;
      for(SMatrix::size_type i = ridx_.at(rowPointer_).first; i < nElements; ++i) { 
         if(cidx_[i] == colPointer_) {    //'iterator' row for
            val = vals_[i];               //current column
         }
      }
   }
   return val;
...
//Helper Functions
//-----------------------------------
int SMatrix::findInsertPos(size_type &row,size_type &col) {
